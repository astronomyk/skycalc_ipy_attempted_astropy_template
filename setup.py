...
PACKAGENAME = metadata.get('package_name', 'skycalc_ipy')
DESCRIPTION = metadata.get('description', 'Access atmospheric spectral data from ESO&#39;s skycalc tool')
AUTHOR = metadata.get('author', 'Kieran Leschinski')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'https://github.com/astronomyk/skycalc_ipy')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.6")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: skycalc_ipy requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
